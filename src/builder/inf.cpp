...
static int _ = program::Resolver::registerBuilder([](app::AppContext &context, program::Resolver &resolver) {
    ...
    resolver.decl("gauss", [&context](float wavelength){return makeInf<float>(context, [wavelength](std::size_t i) {float x = i / wavelength; return std::exp(-x * x);});});
    resolver.decl("gauss", [&context](double wavelength){return makeInf<double>(context, [wavelength](std::size_t i) {double x = i / wavelength; return std::exp(-x * x);});});
});
